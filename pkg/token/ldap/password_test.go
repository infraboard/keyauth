...
func TestConnModifyDNPassword(t *testing.T) {
	...
	if testConfig.PasswordUPN == "" {
		t.Skip("LDAP_PASSWORD_UPN not set")
		return
	}
	...
	//authenticate 1
	status, err = ldap.Authenticate(config, testConfig.PasswordUPN, "Random123!")
	if err != nil {
	...
	//authenticate 2
	status, err = ldap.Authenticate(config, testConfig.PasswordUPN, "Random321!")
	if err != nil {
...
func TestUpdatePassword(t *testing.T) {
	...
	if err = ldap.UpdatePassword(config, testConfig.PasswordUPN, "invalid password", "Random654!"); !strings.Contains(err.Error(), "Password error") {
	...
	if err = ldap.UpdatePassword(config, testConfig.PasswordUPN, "Random456!", randPass); err != nil {
		t.Fatal("Valid password: Expected err to be nil but got:", err)
	}

	status, err = ldap.Authenticate(config, testConfig.PasswordUPN, randPass)
	if err != nil {
