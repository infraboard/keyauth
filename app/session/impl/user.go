...
func (s *service) parseRemoteIPInfo(ip string) (*session.IPInfo, error) {
	if ip == "" {
		return nil, nil
	}
...
// 结束会话后, 禁用该token
func (s *service) closeOldSession(ctx context.Context, tk *token.Token) {
...
func (s *service) DescribeSession(ctx context.Context, req *session.DescribeSessionRequest) (*session.Session, error) {
...
func (s *service) QuerySession(ctx context.Context, req *session.QuerySessionRequest) (*session.Set, error) {
